...
当大家开始学习 React 时，常常寻求一种风格指南。虽然在项目中应用一致的规则是个不错的主意，但是很多规则都很随意。——因此React 对代码风格和规范并没有强烈的要求。

你可以使用不同的类型系统，使用函数声明或者箭头函数，也可以对你的组件属性按字母序或任何你喜欢的顺序来排序。

这样的灵活性允许在项目中 [整合 React](https://reactjs.org/docs/add-react-to-a-website.html) 时，应用已有的一些约定习惯。不过同时，这也会引发无休止的争论。

**_有_一些设计原则是每个组件都应极力遵守的。但我不认为风格指南能很好地体现这些设计原则。接下来，我们先谈谈风格指南，然后[再看看真正有用的设计原则](#编写有弹性的组件)。**
...
在 JavaScript 社区中，有一些由 linter 强制约束的风格指南。我个人的观察是，他们会产比他们价值更多的麻烦。我算不清有多少人向我展示过一些完全有效的代码，然后跟我说 “React 提示这有问题啊！”，但这其实是由他们的 lint 配置产生的！这就导致了三个问题：

* 人们习惯于将 linter 看作是一个过分热心又吵闹的看门人，而不是一个有用的工具。 有用的警告淹没在没用的风格提示海洋中。因此，人们在调试时不看 linter 的提示，而错过有用的信息。此外，不经常写 JavaScript 的人群（例如，设计人员）更难使用代码。

* 对于某种模式人们不会去学习**区分其有效和无效的用法**。例如，有一条流行的规则，禁止在 `componentDidMount` 中调用 `setState`。但如果这个用法总是 “错的”，那 React 根本不会允许它！这就有一个合法的用例，那就是测量 DOM 节点布局——例如，定位 tooltip。我见过有人添加 `setTimeout` 来 “解决” 这条规则，这完全搞错了。
